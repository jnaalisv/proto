...
upload:
    aws s3 cp build/libs/proto.jar s3://bstalk-backenddeploymentbucket-14hs7m8a18wbe/proto.jar
...
tag: package check-env
	docker tag example-app:latest $(ECR_REPO)

run-local: package check-env
	docker run -d -v ~/.aws/:/root/.aws/:ro --net=host -e AWS_REGION=$(AWS_REGION) -e  example-app
